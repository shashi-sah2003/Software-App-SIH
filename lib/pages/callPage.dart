...
class _CallPageState extends State<CallPage> {
  ...
  Future<void> initialize() async {
    ...
    // Start RTMP stream
    const String rtmpUrl = 'rtmp://127.0.0.1:1935/live/stream1';
    _engine.addPublishStreamUrl(rtmpUrl, true);
    ...
    _engine.setEventHandler(RtcEngineEventHandler(
      ...
      firstRemoteVideoFrame: (uid, width, height, elapsed) {
        ...
        streamPublished: (url, error) {
          setState(() {
            final info = 'Stream Published: $url';
            _infoStrings.add(info);
          });
        },
        streamUnpublished: (url) {
          setState(() {
            final info = 'Stream Unpublished: $url';
            _infoStrings.add(info);
          });
        },
    ));
